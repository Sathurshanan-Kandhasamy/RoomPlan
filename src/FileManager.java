...
public class FileManager
{
     ...
     * @param filePath The file location where the data being saved to.
     */
    public void SaveDataToFile(GridData gridData, String filePath)
    {
        try
        {
            //Create a buffered writer to connect to specified file and perform the writing operations.
            BufferedWriter buffer = new BufferedWriter(new FileWriter(filePath));

            //Write the headings data to the file before writing the grid data.
    ...
    public GridData ReadDataFromFile(String filePath)
    {
        ...
        try
        {
            //Create a buffered reader class object to read the file.
            ...
            //Tracks how many lines currently read form the file which will also be the column number of the
    ...
    public void SaveDataToRaf(GridData gridData, String filePath)
    {
        ...
        try
        {
            ...
            //Write the header data before cycling through the grid elements.
            raf.seek(0);
            raf.writeUTF(gridData.name);
            raf.seek(50);
            raf.writeUTF(gridData.date);

            //Tracks how many entries currently written to file
            int counter = 0;
            ...
            for (int x = 0; x < gridData.data.length ; x++)
            {
                for (int y = 0; y < gridData.data[x].length; y++)
                {
                    ...
                    //Increase counter to indicate an entry has been written.
                    counter++;
                }
            }
    ...
    public GridData ReadDataFromRaf(String filePath)
    {
        ...
        //Create a new empty 2D string array to hold the data once it is read back in.
        String[][] data = new String[5][10];
